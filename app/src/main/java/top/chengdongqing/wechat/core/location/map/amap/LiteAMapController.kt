 ...
 */
@Stable
internal class LiteAMapController(
    context: Context,
    isDarkTheme: Boolean,
    ...
    override fun disableMyLocation() {
        scope.launch { mapDeferred.await().setMyLocationEnabled(false) }
    }
    ...
    override fun setOnMapClickListener(listener: (GeoPoint) -> Unit) {
        scope.launch {
            mapDeferred.await().setOnMapClickListener { listener(it.toGeoPoint()) }
        }
    }
    ...
    override fun setOnTouchListener(listener: (MotionEvent) -> Unit) {
        scope.launch {
            mapDeferred.await().setOnMapTouchListener { listener(it) }
        }
    }
