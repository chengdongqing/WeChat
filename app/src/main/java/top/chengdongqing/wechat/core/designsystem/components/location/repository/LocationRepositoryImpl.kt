...
import top.chengdongqing.wechat.core.designsystem.components.location.util.toLatLonPoint
                    ...
                    if (resultCode == AMapException.CODE_AMAP_SUCCESS && result?.query != null) {
                        val items = result.pois.map { poiItem ->
                            LocationInfo(
                                ...
                                distanceMetres = if (current != null) {
                                    AMapUtils.calculateLineDistance(
                                        current,
                                        poiItem.latLonPoint.toLatLng()
                                    ).roundToInt()
                                } else null,
                                coordinate = poiItem.latLonPoint.toLatLng()
                            )
                        }
