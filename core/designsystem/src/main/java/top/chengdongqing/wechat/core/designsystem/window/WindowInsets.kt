package top.chengdongqing.wechat.core.designsystem.window
...
fun rememberKeyboardHeight(
    ...
    val actualMode = remember(mode, view) {
        if (mode == KeyboardHeightMode.Auto) {
            if (isViewInPopup(view)) KeyboardHeightMode.Ime else KeyboardHeightMode.View
        } else {
            mode
        }
    }
    ...
    return when (actualMode) {
        KeyboardHeightMode.Ime -> with(density) { ime.getBottom(density).toDp() }
        KeyboardHeightMode.View -> rememberViewKeyboardHeight(view, density, minHeightThreshold)
    ...
    DisposableEffect(view) {
        val listener = ViewTreeObserver.OnGlobalLayoutListener {
            ...
            when {
                ...
                keyboardHeightPx <= 0 && height > 0.dp -> height = 0.dp
            }
        }
